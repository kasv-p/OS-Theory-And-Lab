...
static void remove_entry_of_inode(struct inode *rip);
...
int fs_readwrite(void)
{
  ...
  if (mode_word == I_IMMEDIATE)
  {
    ...
    else if (rw_flag == WRITING)
    {
      if (position + nrbytes > 32)
      {
        ...
        register struct buf *newbp;
        char *temporary;
        char inode_buffer[40];

        int iter;
        for (iter = 0; iter < f_size; iter++)
        {
          if (iter % 4 == 0)
          {
            temporary = (char *)rip->i_zone + iter;
            inode_buffer[iter] = temporary[iter % 4];
          }
          else
          {
            inode_buffer[iter] = temporary[iter % 4];
        ...
        remove_entry_of_inode(rip);
        rip->i_mode = (I_REGULAR | (rip->i_mode & ALL_MODES));
        mode_word = rip->i_mode & I_TYPE;
        if ((newbp = new_block(rip, (off_t)ex64lo(0))) == NULL)
        {
          return (err_code);
        }
        for (iter = 0; iter < f_size; iter++)
        {
          ((char *)newbp->data)[iter] = inode_buffer[iter];
        }
        MARKDIRTY(newbp);
        put_block(newbp, PARTIAL_DATA_BLOCK);
      }
      else if (position + nrbytes <= 32)
      {
        imm_flag = 1;
      }
      else
  ...
  if (imm_flag == 1)
  {
    ...
    else if (rw_flag != READING)
    {
      ...
      vir_bytes buffer_zone = (vir_bytes)rip->i_zone;

      r = sys_safecopyfrom(VFS_PROC_NR, gid, (vir_bytes)cum_io, buffer_zone + position, (size_t)nrbytes);
      IN_MARKDIRTY(rip);
      if (r == OK)
      {
...
static void remove_entry_of_inode(rip) register struct inode *rip;
{
  ...
  IN_MARKDIRTY(rip);
  for (iter = 0; iter < V2_NR_TZONES; iter++)
  {
    rip->i_zone[iter] = NO_ZONE;
  }
}
